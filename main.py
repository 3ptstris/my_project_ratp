...
# Splitting geo_point column to retrieve lat & lon
idf[['lat', 'lon']] = idf['Geo Point'].str.split(',', expand=True)
idf['lat'] = idf['lat'].str.strip().astype(float)
idf['lon'] = idf['lon'].str.strip().astype(float)
...
,
    ...
    html.H1("Map"),
    dcc.Graph(id = "map_graph",figure=px.scatter_mapbox(idf, lat='lat',lon='lon', hover_name="Geo Point", zoom=6).update_layout(mapbox_style="open-street-map")),
    ])
